...
public final class ServerMessages implements Runnable {
	...
	private final SafeObjectInputStream in;
	private boolean connectionOpen = true;
	public void stopMessages() { connectionOpen = false; }
	
	public ServerMessages(SafeObjectInputStream in) { this.in = in;	}
	...
	public void run() {
		...
		try {
			...
			while (connectionOpen) {
				...
				date = SafeChatClient.DATE_FORMAT.format(incoming.getCreationTime());
				...
				if (incoming instanceof StatusMessage) {
					...
					// Last one, for the comma
					msg += fields[fields.length-1].getName() + "(";
					try {
						
						fields[fields.length-1].setAccessible(true);
						msg += fields[fields.length-1].get(incoming) + ")";
