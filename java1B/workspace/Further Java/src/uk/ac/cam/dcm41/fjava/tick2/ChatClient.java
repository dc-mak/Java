...
public final class ChatClient {
	...
	public static void main(String[] args) {
		...
		try (final Socket sock = new Socket(server, port);
			...
			final DynamicObjectInputStream serverIn = new DynamicObjectInputStream(sock.getInputStream());
			System.out.printf("%s [Client] Connected to %s on port %d.%n",
					DATE_FORMAT.format(new Date()), server,	port);
